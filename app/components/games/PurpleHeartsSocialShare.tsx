...
import { Share2, Twitter, Facebook, Copy, Check, MessageCircle, ExternalLink } from "lucide-react"
import { Button } from "../ui/button"
import { PurpleHeartsScore } from "@/app/lib/supabase-purple-hearts"
import confetti from "canvas-confetti"

interface PurpleHeartsSocialShareProps {
  score: PurpleHeartsScore
  playerRank?: number
  onShare?: () => void
}

export function PurpleHeartsSocialShare({ score, playerRank, onShare }: PurpleHeartsSocialShareProps) {
  const [copied, setCopied] = useState(false)
  const [showShareOptions, setShowShareOptions] = useState(false)

  const gameUrl = "https://loveforbts.com/games/purple-hearts-collector"
  
  const shareText = `💜 I just scored ${score.score} points in Purple Hearts Collector! 
🎯 Accuracy: ${score.accuracy}%
🔥 Best Streak: ${score.best_streak}
⭐ Level: ${score.level}
💕 Hearts Collected: ${score.hearts_collected}
${playerRank ? `🏆 Rank: #${playerRank}` : ''}

Can you catch more hearts than me? Play now! 💜 #BTS #PurpleHearts #ARMY`

  const shareTextShort = `💜 Just scored ${score.score} points in Purple Hearts Collector! Can you beat me? 💜 #BTS #ARMY`
  ...
  const handleRedditShare = () => {
    const title = `Just got ${score.score} points in Purple Hearts Collector!`
          ...
          <div className="bg-white rounded-lg border-2 border-black p-4 space-y-3">
            ...
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-3 mb-4 border border-purple-200">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div className="text-center">
                  <div className="font-bold text-pink-600 text-xl">{score.hearts_collected}</div>
                  <div className="text-gray-600">Hearts</div>
                </div>
              </div>
            </div>
            ...
            <div className="text-xs text-gray-500 text-center mt-3 space-y-1">
              ...
              <p>🏆 Use #PurpleHeartsCollector to join the community!</p>
            </div>
          </div>
        )}
      </motion.div>
